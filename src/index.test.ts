...
describe('Pool Operations', () => {
  ...
  test('should get allowance for adding liquidity', async () => {
    ...
    const amount0Desired = JSBI.BigInt(
      parseUnits(amountA.toFixed(tokenA.decimals), tokenA.decimals).toString()
    );
    const amount1Desired = JSBI.BigInt(
      parseUnits(amountB.toFixed(tokenB.decimals), tokenB.decimals).toString()
    );
